// LIFO sorting is applied to this table. See LIFO_TABLE_SORTING_GUIDE.md for implementation details.
'use client';
...
interface AddCompanyEmployeeDialogProps {
  open: boolean;
  onClose: () => void;
...
export function AddCompanyEmployeeDialog({ open, onClose, companyId, companyName, onSuccess }: AddCompanyEmployeeDialogProps): React.JSX.Element {
  ...
  // Generate unique employee ID and QR code when dialog opens
  React.useEffect(() => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!form.postion.trim()) {
      newErrors.postion = 'Position is required';
      isValid = false;
    }
  ...
  const handleSubmit = async () => {
    if (!companyId) {
      setErrors(prev => ({ ...prev, submit: 'Company ID is required' }));
      return;
    }
    ...
    try {
      const result = await authClient.addCompanyEmployee(companyId, {
        name: form.name,
      ...
      if (result.success) {
        ...
        setErrors(prev => ({ ...prev, submit: result.error || 'Failed to add employee' }));
      }
    } catch (error) {
  ...
  const handleDownloadID = async () => {
    const idCard = document.getElementById('employee-id-card');
    if (!idCard) return;
    ...
    try {
      ...
      canvas.toBlob((blob) => {
        ...
        // Use employee's name for the filename
        const fileName = `${form.name}_ID_Card.png`.replace(/\s+/g, '_');
    ...
    <>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', mx: -1.5 }}>
              ...
              <Box sx={{ width: { xs: '100%', sm: '50%' }, px: 1.5, mb: 2 }}>
                ...
                <TextField
                  ...
                  value={form.postion}
                  onChange={handleChange}
                  error={!!errors.postion}
                  helperText={errors.postion}
                />
              </Box>
                ...
                <CardContent sx={{ p: 3 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 3, alignItems: 'flex-start' }}>
                    ...
                    <Box sx={{ flex: 1 }}>
                      ...
                      <Typography variant="body2" sx={{ mb: 0.5 }}>
                        <strong>Position:</strong> {form.postion}
                      </Typography>
                      <Typography variant="body2" sx={{ mb: 0.5 }}>
                        <strong>National ID:</strong> {form.idNumber}
                      </Typography>
                      <Typography variant="body2" sx={{ mb: 0.5 }}>
                        <strong>Phone:</strong> {form.contactNumber}
                      </Typography>
                      <Typography variant="body2" sx={{ mb: 0.5 }}>
      ...
      >
        <Alert severity="success" sx={{ width: '100%' }}>
          Employee added successfully! You can now download the ID card.
        </Alert>
      </Snackbar>
    </>
  );
