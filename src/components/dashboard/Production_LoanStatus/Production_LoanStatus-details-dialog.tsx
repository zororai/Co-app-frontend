...
export function ProductionLoanDetailsDialog({ open, onClose, userId, onRefresh }: ProductionLoanDetailsDialogProps): React.JSX.Element {
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      // For example: await authClient.updateProductionLoanStatus(userId, status, reason)
      
      // Create the payload with status and reason
      const payload = {
        status: status,
        reason: reason
      };
      ...
      console.log('Updating production loan with payload:', payload);
      ...
      setActionError(`Failed to update production loan status. Please try again.`);
    } finally {
