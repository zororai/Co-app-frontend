...
export function UnassignedShaftsDialog({
  ...
  const handleSaveShaft = async () => {
    ...
    try {
      // Step 1: Update miner ID for the shaft assignment (using company ID as miner ID)
      ...
      // Step 3: Update shaft number for the company (using company-specific endpoint)
      const updateShaftNumberResult = await authClient.updateCompanyShaftNumber(
        customerId,
        selectedShaftData.shaftNumbers
      );
      if (!updateShaftNumberResult.success) {
        throw new Error(updateShaftNumberResult.error || 'Failed to update company shaft number');
      }
      ...
      console.error('Error in company shaft assignment workflow:', error);
