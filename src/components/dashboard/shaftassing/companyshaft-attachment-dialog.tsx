  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        // Third API call - Update shaft number for the company (if needed)
        if (customerId && formData.shaftNumbers) {
          try {
            const response = await fetch(`http://localhost:1000/api/shaft-assignments/company/update-shaftnumber/${customerId}?shaftNumber=${formData.shaftNumbers}`, {
              method: 'PUT',
              headers: {
                'accept': '*/*',
            ...
            if (!response.ok) {
              console.warn('Shaft number update failed but continuing:', response.statusText);
            }
