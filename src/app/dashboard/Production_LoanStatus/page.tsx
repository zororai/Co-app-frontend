...
export default function Page(): React.JSX.Element {
  ...
  const handleExport = (format: 'csv' | 'pdf') => {
    ...
    if (format === 'csv') {
      ...
      a.href = url;
      a.download = `production-loan-status-${tab.toLowerCase()}-${timestamp}.csv`;
      ...
      const doc = new jsPDF();
      doc.setFontSize(20);
      doc.text(`Production Loan Status - ${tab}`, 14, 22);
      doc.setFontSize(12);
      ...
      doc.save(`production-loan-status-${tab.toLowerCase()}-${timestamp}.pdf`);
    }
  };
