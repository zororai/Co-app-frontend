  ...
  if (isLoading) {
    return (
      <Stack alignItems="center" justifyContent="center" sx={{ minHeight: 200 }}>
        <CircularProgress />
        <Typography variant="body2" sx={{ mt: 2 }}>Loading pending shaft assignments...</Typography>
      </Stack>
    );
  }
...
function PushedBackTab({ customers, page, rowsPerPage, onRefresh, isLoading }: TabProps) {
  if (isLoading) {
    return (
      <Stack alignItems="center" justifyContent="center" sx={{ minHeight: 200 }}>
        <CircularProgress />
        <Typography variant="body2" sx={{ mt: 2 }}>Loading pushed back shaft assignments...</Typography>
      </Stack>
    );
  }
  const paginated = applyPagination(customers, page, rowsPerPage);
...
function RejectedTab({ customers, page, rowsPerPage, onRefresh, isLoading }: TabProps) {
  if (isLoading) {
    return (
      <Stack alignItems="center" justifyContent="center" sx={{ minHeight: 200 }}>
        <CircularProgress />
        <Typography variant="body2" sx={{ mt: 2 }}>Loading rejected shaft assignments...</Typography>
      </Stack>
    );
  }
  const paginated = applyPagination(customers, page, rowsPerPage);
...
function ApprovedTab({ customers, page, rowsPerPage, onRefresh, isLoading }: TabProps) {
  if (isLoading) {
    return (
      <Stack alignItems="center" justifyContent="center" sx={{ minHeight: 200 }}>
        <CircularProgress />
        <Typography variant="body2" sx={{ mt: 2 }}>Loading approved shaft assignments...</Typography>
      </Stack>
    );
  }
  const paginated = applyPagination(customers, page, rowsPerPage);
...
export default function Page(): React.JSX.Element {
  ...
  // Function to fetch and update shaft assignment status data
  const fetchShaftAssignmentStatus = React.useCallback(async () => {
    try {
  ...
  const handleExport = () => {
    const headers = [
      'Section', 'Shaft Numbers', 'Medical Fee', 'Registration Fee', 'Contract Start', 'Contract End', 'Status', 'Reason'
    ];
    ...
    a.href = url;
    a.download = `shaft-assignment-status-${tab.toLowerCase()}-${new Date().toISOString().split('T')[0]}.csv`;
    ...
    <Stack spacing={3}>
      <Stack direction="row" spacing={3}>
        <Stack spacing={1} sx={{ flex: '1 1 auto' }}>
          <Typography variant="h4">Shaft Assignment Status</Typography>
          <Tabs
            value={tab}
