...
import { CustomersTable } from '@/components/dashboard/Ore_Recieval/orerec-table';
import type { Customer } from '@/components/dashboard/Ore_Recieval/orerec-table';
  ...
  if (isLoading) {
    return (
      <Stack alignItems="center" justifyContent="center" sx={{ minHeight: 200 }}>
        <CircularProgress />
        <Typography variant="body2" sx={{ mt: 2 }}>Loading pending ore receival...</Typography>
      </Stack>
    );
  }
...
function PushedBackTab({ customers, page, rowsPerPage, onRefresh, isLoading }: TabProps) {
  if (isLoading) {
    return (
      <Stack alignItems="center" justifyContent="center" sx={{ minHeight: 200 }}>
        <CircularProgress />
        <Typography variant="body2" sx={{ mt: 2 }}>Loading pushed back ore receival...</Typography>
      </Stack>
    );
  }
...
export default function Page(): React.JSX.Element {
  ...
  const handleExport = (format: 'csv' | 'pdf') => {
    ...
    if (format === 'csv') {
      ...
      a.href = url;
      a.download = `ore-recieval-${tab.toLowerCase()}-${timestamp}.csv`;
      ...
      const doc = new jsPDF();
      doc.setFontSize(20);
      doc.text(`Check Point Ore Recieval - ${tab}`, 14, 22);
      doc.setFontSize(12);
      ...
      doc.save(`ore-recieval-${tab.toLowerCase()}-${timestamp}.pdf`);
    }
  };
