...
export default function Page(): React.JSX.Element {
  ...
  const handleExport = (format: 'csv' | 'pdf') => {
    ...
    const headers = [
      'Payment Method', 'Amount/Grams', 'Status', 'Reason'
    ];
    ...
    if (format === 'csv') {
      ...
      a.href = url;
      a.download = `transport-cost-status-${tab.toLowerCase()}-${timestamp}.csv`;
      ...
      const doc = new jsPDF();
      doc.setFontSize(20);
      doc.text(`Transport Cost Status - ${tab}`, 14, 22);
      doc.setFontSize(12);
      ...
      doc.save(`transport-cost-status-${tab.toLowerCase()}-${timestamp}.pdf`);
    }
  };
