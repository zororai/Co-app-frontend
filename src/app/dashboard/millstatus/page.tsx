...
import { CustomersTable } from '@/components/dashboard/millstatus/millstatus-onboading-table';
import type { Customer } from '@/components/dashboard/millstatus/millstatus-onboading-table';
  ...
  if (isLoading) {
    return (
      <Stack alignItems="center" justifyContent="center" sx={{ minHeight: 200 }}>
        <CircularProgress />
        <Typography variant="body2" sx={{ mt: 2 }}>Loading pending mill status...</Typography>
      </Stack>
    );
  }
...
function PushedBackTab({ customers, page, rowsPerPage, onRefresh, isLoading }: TabProps) {
  if (isLoading) {
    return (
      <Stack alignItems="center" justifyContent="center" sx={{ minHeight: 200 }}>
        <CircularProgress />
        <Typography variant="body2" sx={{ mt: 2 }}>Loading pushed back mill status...</Typography>
      </Stack>
    );
  }
...
export default function Page(): React.JSX.Element {
  ...
  const handleExport = (format: 'csv' | 'pdf') => {
    ...
    if (format === 'csv') {
      ...
      a.href = url;
      a.download = `mill-status-${tab.toLowerCase()}-${timestamp}.csv`;
      ...
      const doc = new jsPDF();
      doc.setFontSize(20);
      doc.text(`Mill Status - ${tab}`, 14, 22);
      doc.setFontSize(12);
      ...
      doc.save(`mill-status-${tab.toLowerCase()}-${timestamp}.pdf`);
    }
  };
    ...
    <Stack spacing={3}>
      <Stack direction="row" spacing={3} sx={{ alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <Stack spacing={1} sx={{ flex: '1 1 auto' }}>
          <Typography variant="h4">Mill Onboarding Status </Typography>
          <Tabs
            value={tab}
