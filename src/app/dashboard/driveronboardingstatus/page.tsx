...
export default function Page(): React.JSX.Element {
  ...
  // Function to fetch and update driver status data
  const fetchDriverStatus = React.useCallback(async () => {
    try {
      const data = await authClient.fetchPendingCustomers();
      console.log('Fetched driver status data from API:', data);
      ...
      console.log('Normalized driver status data for table:', normalizedData);
  ...
  // Function to refresh the driver status data
  ...
  const handleExport = (format: 'csv' | 'pdf') => {
    ...
    if (format === 'csv') {
      ...
      a.href = url;
      a.download = `driver-onboarding-status-${tab.toLowerCase()}-${timestamp}.csv`;
      ...
      const doc = new jsPDF();
      doc.setFontSize(20);
      doc.text(`Driver Onboarding Status - ${tab}`, 14, 22);
      doc.setFontSize(12);
      ...
      doc.save(`driver-onboarding-status-${tab.toLowerCase()}-${timestamp}.pdf`);
    }
  };
    ...
    <Stack spacing={3}>
      <Stack direction="row" spacing={3} sx={{ alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <Stack spacing={1} sx={{ flex: '1 1 auto' }}>
          <Typography variant="h4">Driver Onboarding Status Health  </Typography>
          <Tabs
            value={tab}
